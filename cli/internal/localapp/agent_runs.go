...
func recordAgentThought(run *agentRunState, id, text string) {
	...
	if id == "" {
		id = fmt.Sprintf("thought-%d", len(run.Thoughts))
	}
	for i := range run.Thoughts {
		if run.Thoughts[i].ID == id {
			run.Thoughts[i].Text = text
			return
		}
	}
...
func (s *Server) loadAgentRuns() error {
	...
	if len(data) > 0 {
		if err := json.Unmarshal(data, &runs); err != nil {
			return fmt.Errorf("read agent run journal: %w", err)
		}
	}
...
func (s *Server) publishAgentRuns(persist bool) {
	...
	if persist {
		...
		if err != nil {
			s.logCLIOutput("Could not save agent run history: %v\n", err)
		}
	}
...
func (s *Server) finishAgentRun(id, status, message string) {
	s.agentMu.Lock()
	run := s.agentRun
	if !activeAgentRun(run) || run.ID != id {
		s.agentMu.Unlock()
		return
	}
...
func (s *Server) startAgentPrompt(request agentPromptRequest) error {
	if strings.TrimSpace(request.Prompt) == "" || request.RunID == "" || request.ClientID == "" {
		return fmt.Errorf("prompt, runId, and clientId are required")
	}
	if request.DocumentID != s.document.Manifest().DocumentID {
		return fmt.Errorf("the canvas document has changed")
	}
	s.agentMu.Lock()
	for _, run := range s.agentHistory {
	...
	if s.agentAuthChanging {
		s.agentMu.Unlock()
		return fmt.Errorf("Agent authentication is changing; wait for it to reconnect")
	}
	if client == nil || s.agentStatus.State != "ready" {
	...
	if activeAgentRun(s.agentRun) {
		s.agentMu.Unlock()
		return fmt.Errorf("the Agent is already working; stop the current run first")
	}
	...
	if len(s.agentHistory) > 30 {
		s.agentHistory = s.agentHistory[len(s.agentHistory)-30:]
	}
	s.agentMu.Unlock()
	s.publishAgentRuns(true)
	s.workerMu.Lock()
	if s.closing {
		s.workerMu.Unlock()
		s.finishAgentRun(run.ID, "cancelled", "Kavla is closing.")
	...
	go func() {
		...
		defer func() {
			...
			if ctx.Err() == context.DeadlineExceeded {
				s.cancelAgentRun(run.ID, "Agent run exceeded its 15-minute limit. Existing canvas work has been kept.")
			}
		}()
		if ctx.Err() != nil {
			return
		...
		if err != nil {
			s.finishAgentRun(run.ID, "failed", err.Error())
			return
		}
		history := ""
		if !resumed {
		...
		s.agentMu.Lock()
		if !activeAgentRun(run) || s.agentRun != run {
			s.agentMu.Unlock()
			return
		}
...
func (s *Server) stopAgentRun(id, status, reason string) {
	...
	if !activeAgentRun(run) || (id != "" && run.ID != id) {
		s.agentMu.Unlock()
		return
	}
	...
	if client != nil && threadID != "" && turnID != "" {
		...
		if err := client.InterruptTurn(ctx, threadID, turnID); err != nil {
			s.logCLIOutput("Could not interrupt agent: %v\n", err)
		}
	}
}
...
func (s *Server) handleAgentToolCall(requestID json.RawMessage, params json.RawMessage) {
	...
	run, client := s.agentRun, s.agentClient
	reject := ""
	if err != nil || request.CallID == "" {
		reject = "Invalid canvas tool call."
	} else if !activeAgentRun(run) || run.ThreadID != request.ThreadID || (run.TurnID != "" && run.TurnID != request.TurnID) {
		reject = "This agent run is no longer active."
	} else if (request.Namespace != "" && request.Namespace != "kavla") || !isAllowedAgentCanvasTool(request.Tool) {
		reject = "Only Kavla canvas tools are available."
	}
	if reject == "" {
		for _, previous := range run.Tools {
			if previous.CallID == request.CallID {
				...
				s.agentMu.Unlock()
				if client != nil && status == "completed" {
					_ = client.RespondToTool(requestID, success, result)
				}
				return
			}
		}
		if len(run.Tools) >= agent.MaxToolCallsOrDefault(run.MaxToolCalls) {
			reject = "The run has reached its tool budget. Finish with the best available evidence."
		}
	}
	if reject != "" {
		s.agentMu.Unlock()
		if client != nil {
			_ = client.RespondToTool(requestID, false, map[string]string{"error": reject})
		}
		return
	}
	...
	if progress, ok := request.Arguments["progress"].(string); ok {
		recordAgentThought(run, "tool:"+request.CallID, progress)
	}
	...
	if request.Tool == "create_lens" || request.Tool == "update_lens" {
		return
	}
	go func() {
		...
		case <-timer.C:
			...
			s.agentMu.Unlock()
			if pending {
				s.cancelAgentRun(runID, "A canvas tool timed out. The existing canvas work has been kept.")
			}
		}
	}()
}
...
func (s *Server) handleAgentToolClaim(w http.ResponseWriter, r *http.Request) {
	...
	if json.NewDecoder(r.Body).Decode(&request) != nil {
		writeAPIError(w, 400, fmt.Errorf("invalid claim"))
		return
	}
	s.agentMu.Lock()
	run := s.agentRun
	...
	if activeAgentRun(run) && run.ID == request.RunID && run.ClientID == request.ClientID {
		for _, call := range run.Tools {
			if call.CallID == request.CallID && call.Status == "pending" {
				call.Status = "running"
				run.Revision++
				claimed = true
				break
...
func (s *Server) acceptAgentToolResult(request agentToolResultRequest) error {
	s.agentMu.Lock()
	run, client := s.agentRun, s.agentClient
	if !activeAgentRun(run) || run.ID != request.RunID || run.ClientID != request.ClientID || client == nil {
		s.agentMu.Unlock()
		return fmt.Errorf("this agent run is no longer active")
	}
	...
	if call == nil {
		s.agentMu.Unlock()
		return fmt.Errorf("unknown tool call")
	}
	...
	if call.Status != "running" {
		s.agentMu.Unlock()
		return fmt.Errorf("tool call must be claimed before returning a result")
	}
	value := request.Result
	if !request.Success && value == nil {
...
func (s *Server) handleAgentGenerateLens(w http.ResponseWriter, r *http.Request) {
	...
	if json.NewDecoder(r.Body).Decode(&request) != nil {
		writeAPIError(w, 400, fmt.Errorf("invalid generation request"))
		return
	}
	s.agentMu.Lock()
	run, client := s.agentRun, s.agentClient
	if !activeAgentRun(run) || run.ID != request.RunID || run.ClientID != request.ClientID || client == nil {
		s.agentMu.Unlock()
		writeAPIError(w, 409, fmt.Errorf("this agent run is no longer active"))
		return
	}
	target := "lens"
	if supplied, ok := request.Context.(map[string]interface{}); ok {
		if id, ok := supplied["targetShapeId"].(string); ok && id != "" {
			target = id
		}
	}
	if run.LensAttempts == nil {
		run.LensAttempts = make(map[string]int)
	}
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) || ctx.Err() == context.DeadlineExceeded {
			...
			if parent.Err() == context.DeadlineExceeded {
				message = fmt.Errorf("Agent run exceeded its 15-minute limit during Lens generation. Existing canvas work has been kept.")
			}
