...
func (s *Server) loadAgentConfig() error {
	...
	if err != nil {
		return fmt.Errorf("locate Agent config: %w", err)
	}
	...
	if err := yaml.Unmarshal(data, &saved); err != nil {
		return fmt.Errorf("invalid Agent config in %s: %w", path, err)
	}
	if saved.Mode != "codex" && saved.Mode != "apiKey" {
		return fmt.Errorf("Agent config %s must select mode codex or apiKey", path)
	}
	...
	if saved.MaxToolCalls != nil {
		provider.MaxToolCalls = *saved.MaxToolCalls
	}
	if err := agent.ValidateMaxToolCalls(provider.MaxToolCalls); err != nil {
		return fmt.Errorf("invalid Agent config %s: %w", path, err)
	}
...
func (s *Server) saveAgentConfig(mode string, provider agent.APIConfig) error {
	...
	if err := atomicWriteFile(path, data, 0600); err != nil {
		return fmt.Errorf("save Agent config %s: %w", path, err)
	}
	return nil
}
