...
func TestAgentRoutesAndRuntimeStream(t *testing.T) {
	...
	if auth.Code != http.StatusOK { t.Fatalf("auth route: %d %s", auth.Code, auth.Body.String()) }
	var settings map[string]interface{}
	if err := json.Unmarshal(auth.Body.Bytes(), &settings); err != nil { t.Fatal(err) }
	if settings["mode"] != "codex" { t.Fatalf("unexpected provider mode: %v", settings) }
	...
	for _, stream := range []string{"cli", "agent"} {
		...
		if err := wsjson.Read(ctx, connection, &event); err != nil { t.Fatal(err) }
		if event.Stream != stream || event.Name != "snapshot" { t.Fatalf("expected %s snapshot, got %+v", stream, event) }
	}
	s.broadcastAgentRuntimeEvent(cliRuntimeEvent{name: "status", data: map[string]string{"state": "ready", "message": "Connected"}})
	var event runtimeSocketEvent
	if err := wsjson.Read(ctx, connection, &event); err != nil { t.Fatal(err) }
	if event.Stream != "agent" || event.Name != "status" { t.Fatalf("expected agent status, got %+v", event) }
