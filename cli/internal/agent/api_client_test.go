...
func TestAPIClientToolLoop(t *testing.T) {
	...
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if requests.Add(1) == 1 {
			if !strings.Contains(string(body.Messages[1]["content"]), "Earlier conversation") {
				t.Error("missing fallback history")
			}
		...
		if len(body.Messages) != 5 {
			t.Errorf("expected assistant and both tool results, got %d messages", len(body.Messages))
		} else {
			if string(body.Messages[2]["reasoning_content"]) != `"preserve provider reasoning"` {
				t.Error("lost provider reasoning field")
			}
			if string(body.Messages[3]["tool_call_id"]) != `"call-1"` || string(body.Messages[4]["tool_call_id"]) != `"call-2"` {
				t.Error("tool results out of order")
			}
			if !strings.Contains(string(body.Messages[3]["content"]), "shape:note") {
				t.Error("missing canvas tool result")
			}
		}
	...
	client, err = NewAPIClient(context.Background(), APIConfig{BaseURL: provider.URL + "/v1", APIKey: "test-key", Model: "test-model", Headers: map[string]string{"cf-aig-gateway-id": "kavla"}},
		func(method string, raw json.RawMessage) {
			...
			if method == "turn/completed" {
				done <- event["turn"].(map[string]interface{})
			}
			if method == "item/completed" {
				texts <- event["item"].(map[string]interface{})["text"].(string)
			}
		},
		func(id, raw json.RawMessage) {
			var call struct {
				Namespace, Tool string
			...
			if call.Namespace != "kavla" {
				t.Error("missing canvas namespace")
			}
			if err := client.RespondToTool(id, true, map[string]string{"shapeId": "shape:note"}); err != nil {
				t.Error(err)
			}
		})
	if err != nil {
	...
	if err != nil || resumed {
		t.Fatalf("start thread: resumed=%v, err=%v", resumed, err)
	}
	prompt, err := BuildPrompt("Create a note", map[string]interface{}{}, "Earlier conversation")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := client.StartTurn(context.Background(), thread, prompt); err != nil {
		t.Fatal(err)
	}
	select {
	case turn := <-done:
	...
	case <-time.After(3 * time.Second):
		t.Fatal("tool loop did not finish")
	}
	if requests.Load() != 2 {
		t.Fatalf("expected two API calls, got %d", requests.Load())
	}
	if text := <-texts; text != "Created the note." {
		t.Fatal(text)
	}
}
...
func TestAPIClientLensGeneration(t *testing.T) {
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["stream"] != false || body["tools"] != nil {
			t.Error("focused generation must use complete text without tools")
		}
		messages := body["messages"].([]interface{})
		instructions := messages[0].(map[string]interface{})["content"].(string)
		content := ""
		if instructions == lensDeveloperInstructions {
			content = `{"code":"export default function Lens() { return null; }"}`
		}
...
func TestAPIClientInterruptsPendingTool(t *testing.T) {
	...
	case <-time.After(3 * time.Second):
		t.Fatal("tool was not requested")
	}
	if err := client.InterruptTurn(context.Background(), thread, turn); err != nil {
		t.Fatal(err)
	}
	select {
	case raw := <-done:
		if !strings.Contains(string(raw), `"status":"interrupted"`) {
			t.Fatal(string(raw))
		}
	case <-time.After(3 * time.Second):
		t.Fatal("waiting tool did not cancel")
	}
	if err := client.RespondToTool(requestID, true, nil); err == nil {
		t.Fatal("accepted a late tool result")
	}
	if _, _, err := client.StartOrResumeThread(context.Background(), thread, "test-model"); err != nil {
		t.Fatalf("cannot start after interruption: %v", err)
	}
}
...
func TestAPIClientFinalizesAtToolBudget(t *testing.T) {
	...
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if step <= toolLimit {
			...
			if body["tools"] != nil {
				t.Error("tools still enabled after budget exhaustion")
			}
	...
	client, err = NewAPIClient(context.Background(), APIConfig{BaseURL: provider.URL, Model: "test-model", MaxToolCalls: toolLimit},
		...
		func(id, raw json.RawMessage) {
			calls.Add(1)
			if err := client.RespondToTool(id, true, nil); err != nil {
				t.Error(err)
			}
		})
	if err != nil {
	...
	if _, err := client.StartTurn(context.Background(), thread, "Analyze"); err != nil {
		t.Fatal(err)
	}
	select {
	case raw := <-done:
	...
	case <-time.After(3 * time.Second):
		t.Fatal("tool budget did not terminate")
	}
	if requests.Load() != toolLimit+1 || calls.Load() != toolLimit {
		t.Fatalf("requests=%d, calls=%d", requests.Load(), calls.Load())
	}
}
...
func TestAPIClientCancellation(t *testing.T) {
	...
	case <-cancelled:
	case <-time.After(3 * time.Second):
		t.Fatal("provider request is still running")
	}
}
...
func TestAPILensGenerationKeepsLongerDeadline(t *testing.T) {
	...
	client.httpClient.Transport = apiTestTransport(func(request *http.Request) (*http.Response, error) {
		...
		if !ok || remaining < 8*time.Minute || remaining > 9*time.Minute {
			t.Errorf("HTTP client shortened Lens deadline: %s", remaining)
		}
	...
	defer cancel()
	if _, err := client.GenerateLens(ctx, "test-model", "Generate a Lens", nil); err != nil {
		t.Fatal(err)
	}
}
...
func TestAPIClientRejectsIncompleteResponses(t *testing.T) {
	for _, body := range []string{
		...
		t.Run(body, func(t *testing.T) {
			...
			defer client.Close()
			if _, err := client.complete(context.Background(), "test-model", nil, nil); err == nil {
...
func TestAPIClientRedactsErrorsAndRejectsRedirects(t *testing.T) {
	...
	for _, status := range []int{http.StatusUnauthorized, http.StatusTemporaryRedirect} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			...
			if err == nil || strings.Contains(err.Error(), "key-secret") || strings.Contains(err.Error(), "gateway-secret") {
				t.Fatalf("expected a redacted provider error, got %v", err)
			}
		})
	}
	if forwarded.Load() {
		t.Fatal("credentials were forwarded through a redirect")
	}
}
