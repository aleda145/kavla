...
export function AgentRuntime() {
  ...
  const onActivityShape = useCallback(
    (shapeId: string) => {
    ...
    () =>
      editor.sideEffects.registerAfterChangeHandler("shape", (previous, next) => {
        ...
        if (
          ...
          previousSize.w === nextSize.w &&
          previousSize.h === nextSize.h
        )
          return;
        editor.run(
          () => {
            moveAgentBlobToShape(editor, next.id, blob.props.currentJobId, blob.props.lastMessage, {
              status: blob.props.status,
            });
          },
          { history: "ignore" }
        );
  ...
  useEffect(() => {
    ...
    const appendOnce = (
      ...
      entry: Parameters<typeof appendAgentChatEntry>[1]
    ) => {
      ...
      if (existing) {
        if (entry.role === "thought" && existing.text !== entry.text) {
          updateAgentChat(editor, {
            entries: entries.map((item) => (item.id === existing.id ? { ...item, text: entry.text } : item)),
          });
        }
        return;
      }
    ...
    for (const run of visibleRuns) {
      ...
      for (const call of run.tools) {
        ...
        appendOnce(run.id, call.callId, {
          role: "query",
          text: `Created: [${label}](${shapeId})`,
          shapeIds: [shapeId],
        });
      }
      if (!isAgentRunActive(run)) {
        ...
        if (observedActiveRuns.current.delete(run.id) && run.status === "completed") {
          try {
            if (ensureAgentFinalQueryTable(editor, run)) {
              void stageCanvas(editor).catch((error) =>
                appendOnce(run.id, "final-result-table", {
                  role: "error",
                  text: `Could not save the final result table: ${error instanceof Error ? error.message : String(error)}`,
                })
              );
            }
          } catch (error) {
            appendOnce(run.id, "final-result-table", {
              role: "error",
              text: `Could not show the final result table: ${error instanceof Error ? error.message : String(error)}`,
            });
          }
        }
        for (const shape of editor.getCurrentPageShapes()) {
          ...
          if (lens.props.jobId === run.id && ["generating", "repairing"].includes(lens.props.generationStatus)) {
            editor.updateShape<LensShape>({
              id: lens.id,
              type: "lens-shape",
              props: { generationStatus: "error", error: run.error || "Lens generation ended before returning code." },
            });
          }
        }
        const shapeIds = [
          ...new Set([
            ...
            ...run.tools
              ...
              .flatMap((call) => [call.result?.shapeId, call.result?.linkedTableId])
        ...
        const text =
          run.status === "completed" && summary?.type === "summary-shape"
            ? `${(summary as SummaryShape).props.answer.trim()} [Summary](${summary.id})`
            : run.text || "Done.";
        if (run.text || run.status === "completed")
          appendOnce(run.id, undefined, { role: "assistant", text, shapeIds });
        if (run.status !== "completed")
          appendOnce(run.id, undefined, {
            role: run.status === "failed" ? "error" : "event",
            text: run.error || `Agent ${run.status}.`,
            shapeIds,
          });
      }
    }
    ...
    const next = {
      ...
      streamingText: active?.text || "",
      activity: active?.activity || null,
      threadId: latest?.threadId || agent.props.threadId,
    };
    if (Object.entries(next).some(([key, value]) => agent.props[key as keyof typeof next] !== value))
      updateAgentChat(editor, next);
    editor.run(
      () => {
        if (active) {
          ...
          setAgentBlobStatus(
            editor,
            blob?.props.targetShapeIds?.length ? "working" : "thinking",
            active.id,
            active.activity
          );
    ...
    if (active) {
      const call =
        [...active.tools].reverse().find((call) => call.status === "pending" || call.status === "running") ||
        active.tools[active.tools.length - 1];
  ...
  useEffect(() => {
    ...
    for (const call of run.tools.filter((call) => call.status === "pending").slice(0, 1)) {
      ...
      void (async () => {
        const claim = await agentRequest<{ claimed: boolean }>(
          "tool-claims",
          { runId: run.id, clientId: agentClientId, callId: call.callId },
          signal
        );
        ...
        try {
          ...
          if (typeof target === "string") onActivityShape(target);
          result = await executeAgentCanvasTool(
            editor,
            call.tool,
            call.arguments,
            onActivityShape,
            createAgentToolEnvironment(run, signal, data)
          );
        } catch (error) {
          signal.throwIfAborted();
        ...
        const payload = {
          ...
          result,
          error: result.ok === false ? String(result.error || "The canvas tool failed.") : undefined,
        };
        try {
        ...
        appendAgentChatEntry(editor, {
          role: "error",
          runId: run.id,
          toolCallId: call.callId,
          text: `Agent tool delivery failed: ${error instanceof Error ? error.message : String(error)}`,
        });
