...
export function appendAgentChatEntry(editor: Editor, entry: Omit<AgentChatEntry, "id" | "createdAt">) {
...
export function getAgentContextShapeIds(editor: Editor): TLShapeId[] {
  const selected = editor
    .getSelectedShapeIds()
    .filter((id) => id !== AGENT_CHAT_SHAPE_ID && editor.getShape(id)?.type !== "agent-blob");
  if (selected.length > 0) return selected.slice(0, 8);
  const recent = [
    ...new Set(
      (getAgentChat(editor)?.props.entries ?? [])
        .slice(-30)
        .reverse()
        .flatMap((entry) => [...(entry.shapeIds ?? []), ...(entry.contextShapeIds ?? [])])
    ),
