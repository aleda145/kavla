...
export async function runLensTool(
  editor: Editor,
  args: Record<string, unknown>,
  env: AgentToolEnvironment,
  editing: boolean,
  onActivityShape?: (id: string) => void
  ...
  const visualPrompt =
    (typeof args.visualPrompt === "string" ? args.visualPrompt.trim() : "") || (editing ? env.prompt.trim() : "");
  ...
  if (!existing) {
    ...
    editor.createShape<LensShape>({
      id,
      type: "lens-shape",
      x: placement.x,
      y: placement.y,
      props: {
        name: getUniqueName(editor, typeof args.name === "string" ? args.name : "Lens"),
    ...
    editor.updateShape<LensShape>({
      id,
      type: "lens-shape",
      props: { generationStatus: "generating", jobId: env.runId, error: null },
    });
  }
  onActivityShape?.(id);
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      const generated = await env.generateLens(visualPrompt, {
        targetShapeId: id,
        userRequest: env.prompt,
        sourceName: query.props.name,
        schema,
        rowCount: query.props.lastRunStats?.rowCount,
        sampleRows: data.slice(0, 5),
        isSampled: false,
        sourceSql: query.props.text,
        currentCode: attemptCode,
        currentDataSql: attemptDataSql,
        dataIntent: args.dataIntent,
        currentTitle: current.props.title,
        currentDescription: current.props.description,
        width: current.props.w - 48,
        height: current.props.h - 46,
        attempt,
      ...
      if (attempt > 1 && attemptCode === previousCode && attemptDataSql === previousDataSql)
        throw new Error("The Lens generator repeated the same failed code. Stopped without another attempt.");
      try {
        await validateGeneratedChartWidget({
          isSampled: false,
          code: attemptCode,
          dataSql: attemptDataSql,
          rows: data,
          columns: schema.map((column) => column.name),
          columnTypes: Object.fromEntries(schema.map((column) => [column.name, column.type])),
          sourceName: query.props.name,
          width: current.props.w - 48,
          height: current.props.h - 46,
        });
        ...
        if (latest.props.code !== codeBeforeGeneration || latest.props.dataSql !== sqlBeforeGeneration)
          throw new Error("The Lens was edited during generation; the Agent kept your edit.");
        const latestQuery = editor.getShape<SQLTextAreaShape>(query.id);
        if (
          !latestQuery ||
          latestQuery.props.text !== query.props.text ||
          latestQuery.props.isDirty ||
          latestQuery.props.stale
        )
          throw new Error("The source query changed during generation.");
        editor.updateShape<LensShape>({
          id,
          type: "lens-shape",
          props: {
            code: attemptCode,
            dataSql: attemptDataSql,
            title: generated.title || latest.props.name,
        ...
        if (
          lastError.includes("kept your edit") ||
          lastError.includes("was removed") ||
          lastError.includes("source query changed")
        )
          throw error;
        if (attempt < 2 && editor.getShape(id))
          editor.updateShape<LensShape>({
            id,
            type: "lens-shape",
            props: { generationStatus: "repairing", error: lastError },
          });
      }
    }
    throw new Error(lastError || "Lens generation failed after two attempts.");
  } catch (error) {
    const message = env.signal.aborted
      ? "Lens generation stopped."
    ...
    return {
      ...
      stopRun: true,
      guidance:
        "Stop this run. Do not generate another Lens or call update_lens again. The error and available code remain on the canvas; retry only when the user explicitly asks.",
    };
  }
}
