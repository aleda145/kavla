...
let auth: AgentAuth = { maxToolCalls: 50, baseUrl: "https://api.openai.com/v1", model: "gpt-4.1", hasHeaders: false, mode: "codex", hasApiKey: false, hasEnvironmentKey: false, keySource: "" };
...
export function notifyAgentAuth(value: unknown) {
  ...
  auth = { maxToolCalls: typeof next.maxToolCalls === "number" ? next.maxToolCalls : auth.maxToolCalls, baseUrl: typeof next.baseUrl === "string" ? next.baseUrl : auth.baseUrl, model: typeof next.model === "string" ? next.model : auth.model, hasHeaders: next.hasHeaders === true, mode: next.mode, hasApiKey: next.hasApiKey, hasEnvironmentKey: next.hasEnvironmentKey === true, keySource: next.keySource === "config" || next.keySource === "session" || next.keySource === "environment" ? next.keySource : "" };
