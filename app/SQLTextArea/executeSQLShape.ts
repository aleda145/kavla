...
export async function executeSQLShape(editor: Editor, data: LocalServerContextType, id: TLShapeId, sql: string, signal: AbortSignal): Promise<SQLShapeRunResult> {
  ...
  try {
    signal.throwIfAborted();
    const output = await data.runRemoteQuery({ sql: buildRemoteSQLFromDag(sql, orderedDependencies), sourceName: executionState.sourceName, sourceType: executionState.sourceType, sourceNative: executionState.sourceNativePreview, shapeId: id, queryName: shape.props.name });
    signal.throwIfAborted();
