...
export class LensUtil extends ShapeUtil<LensShape> {
  ...
  getGeometry(shape: LensShape) {
    return new Rectangle2d({ width: shape.props.w, height: shape.props.h, isFilled: true });
  }
  override onResize(shape: LensShape, info: TLResizeInfo<LensShape>) {
    return resizeBox(shape, info);
  }
  indicator(shape: LensShape) {
  ...
  component(shape: LensShape) {
    ...
    const rows = useQueryResultRows({
      sourceShapeId: source?.id || null,
      sourceTableName: source?.props.name || null,
      schema: source?.props.outputSchema,
      revision: source?.props.lastRunStats,
      normalizeRow: normalizeChartRow,
      serverResultShapeId: source?.id ?? null,
      restoreServerResult,
    });
    const widgetKey = `${shape.id}:${shape.props.generatedAt}:${shape.props.code}:${shape.props.dataSql}`;
    const update = (props: Partial<LensShape["props"]>) =>
      editor.updateShape<LensShape>({ id: shape.id, type: "lens-shape", props });
    const reportError = (error: string) => {
      const current = editor.getShape<LensShape>(shape.id);
      if (
        !current ||
        ["generating", "repairing"].includes(current.props.generationStatus) ||
    ...
    const status = !source
      ? "Connect this Lens to a visible query."
      : source.props.isDirty || source.props.stale
        ? "Run the source query to refresh this Lens."
        : rows.isLoading
          ? "Loading query results…"
          : rows.error ||
            (shape.props.generationStatus === "error"
              ? shape.props.error || "Lens failed. Edit the code or ask the Agent to repair it."
              : null) ||
            (!shape.props.code
              ? generating
                ? "Generating Lens…"
                : shape.props.error || "Ask the Agent to create a Lens."
              : null);
    return (
      <HTMLContainer
        id={shape.id}
      ...
      >
        <LensHeader
          title={shape.props.title || shape.props.name}
          description={shape.props.description}
          hasRetryStatus={generating}
        />
        <LensBody
          isSampled={rows.isTruncated}
          code={shape.props.code}
          dataSql={shape.props.dataSql}
          defaultDataSql={`SELECT * FROM ${quoteIdentifier(source?.props.name || "data")}`}
          shapeId={shape.id}
          sourceName={source?.props.name || "data"}
          data={rows.data}
          columns={rows.columns}
          columnTypes={rows.columnTypes}
          widgetKey={widgetKey}
          widgetWidth={Math.max(1, shape.props.w - 48)}
          widgetHeight={Math.max(1, shape.props.h - 74)}
          viewMode={viewMode}
          statusMessage={status}
          isStatusError={Boolean(rows.error || shape.props.error)}
          onShowCode={() => setViewMode("code")}
          onShowLens={() => setViewMode("lens")}
          onShowSql={() => setViewMode("sql")}
          onCodeChange={(code) => update({ code, error: null, generationStatus: "ready", retryCount: 0 })}
          onDataSqlChange={(dataSql) =>
            update({ dataSql: dataSql.trim() ? dataSql : null, error: null, generationStatus: "ready", retryCount: 0 })
          }
          onDataSqlFormat={(sql) => {
            try {
              update({
                dataSql: format(
                  sql || shape.props.dataSql || `SELECT * FROM ${quoteIdentifier(source?.props.name || "data")}`,
                  { language: "duckdb" }
                ),
              });
            } catch (error) {
          ...
          onWidgetError={reportError}
          onDataSqlError={reportError}
        />
        <div
          style={{
            fontSize: 10,
            ...
            color: shape.props.error ? "#991b1b" : "#57534e",
          }}
        >
          {generating ? "Generating Lens… " : shape.props.error ? `${shape.props.error} ` : ""}
          {`${rows.data.length.toLocaleString()} rows from ${source?.props.name || "query"}.`} Presentation SQL uses
          these rows.
        </div>
      </HTMLContainer>
    );
